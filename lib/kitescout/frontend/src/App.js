...
function App() {
  ...
  const getSettings = async () => {
    ...
    if (settings.current?.displayUnits?.includes("mmol")) {
      unitConverter.current = 18;
      settings.current.bgTargTop = settings.current.bgTargTop / 18;
      settings.current.bgTargBot = settings.current.bgTargBot / 18;
    }
  }
  ...
  const checkIfMoreDataIsNeeded = async (c) => {
    ...
    if (c.chart.scales.x.min < firstDataDate.getTime() + offset) {
      ...
      var toTime = new Date(c.chart.scales.x.max + offset);
      await getData(fromTime, toTime, true);
      //c.chart.update('none');
    } else if (c.chart.scales.x.max < lastDataDate.getTime() + offset) {
      var fromTime = new Date(c.chart.scales.x.min - offset);
      var toTime = new Date(lastDataDate.getTime() + offset);
      if (toTime.getTime() < c.chart.scales.x.max) {
        toTime = new Date(c.chart.scales.x.max + offset);
      }
      await getData(fromTime, toTime, true);
    }
  ...
  const chartOptions = {
    ...
    scales: {
      ...
      y: {
        position: 'left',
        min: -10,
        max: sgv.current == undefined ? Number(settings.current.bgTargTop) * 2 : Math.ceil(Math.max(...sgv.current.map(x => x.bg)) / (5 * (19 - unitConverter.current)) * 5 * (19 - unitConverter.current) + 5 * (19 - unitConverter.current)),
        ticks: {
          min: 0,
          stepSize: 1,
          ...
          <tbody>
            <tr>
              ...
              <td rowspan="2" style={{ textAlign: "left", verticalAlign: "bottom" }}>
                <span style={currInfo?.dBg.includes("-") ? { color: RGB_red } : { color: RGB_green }}>({
                  currInfo ? settings.current?.displayUnits?.includes("mmol") ? Number(currInfo?.dBg).toFixed(1) : Number(currInfo?.dBg).toFixed(0) : ""
                })</span>
              </td>
              <td>IOB:</td>
